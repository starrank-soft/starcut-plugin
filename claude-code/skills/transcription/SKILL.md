...
→ mcp__plugin_starcut_starcut__client_call with command: get_transcript
→ mcp__plugin_starcut_starcut__run_task with task: transcribe when prepared
...
| Need | Call |
|---|---|
| Get a cached transcript or prepare canonical audio | `mcp__plugin_starcut_starcut__client_call` with `command: "get_transcript"` |
| Materialize audio or select a source range | `mcp__plugin_starcut_starcut__client_call` with `command: "extract_audio"` |
| Run ASR on an exact ready Audio Artifact | `mcp__plugin_starcut_starcut__run_task` with `task: "transcribe"` |
| Generate speech | `mcp__plugin_starcut_starcut__query` with `audio.tts`, then `mcp__plugin_starcut_starcut__run_task(generate)` |
| Persist captions | StarCut Node tools on the target Timeline |

Use `mcp__plugin_starcut_starcut__client_call` with `command: "get_transcript"` by default for both
...
it returns `prepared`, call `mcp__plugin_starcut_starcut__run_task` once with the returned
...
`mcp__plugin_starcut_starcut__poll` with its `taskId`; do not call `get_transcript` again to
poll or start duplicate work.

## Get a Transcript

Locate the source with `mcp__plugin_starcut_starcut__glob`, inspect it with
`mcp__plugin_starcut_starcut__head`, and pass its exact Artifact ID:
...
`mcp__plugin_starcut_starcut__run_task`. Optional ASR fields are `modelId`, `language`,
`diarize`, and `keyterms`; add them to those Task params, not to
`mcp__plugin_starcut_starcut__client_call`. Omit `modelId` to use the configured ASR default.
Call `mcp__plugin_starcut_starcut__query` for `audio.asr` only when model selection matters.
...
Use `mcp__plugin_starcut_starcut__client_call` with `command: "extract_audio"` only when the user
...
`mcp__plugin_starcut_starcut__run_task` with `task: "transcribe"` on the returned ready Audio Artifact using its exact
...
4. Call `mcp__plugin_starcut_starcut__client_call` with `command: "get_transcript"` on the generated Audio Artifact.
5. If prepared, call `mcp__plugin_starcut_starcut__run_task` with the returned Task input.

...
| Generate speech | `mcp__plugin_starcut_starcut__query` with `kind: "model"` and intent `audio.tts`, then `mcp__plugin_starcut_starcut__run_task(generate)` |
...
`mcp__plugin_starcut_starcut__head`, and pass its exact path:
...
Call `mcp__plugin_starcut_starcut__query` with `kind: "model"` and intent `audio.asr` only
...
`mcp__plugin_starcut_starcut__run_task` with `task: "transcribe"` and the returned Audio path.

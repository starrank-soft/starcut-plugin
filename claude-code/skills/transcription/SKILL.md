...
`mcp__plugin_starcut_starcut__poll` with its returned `toolCallId`; do not call
...
4. Call `mcp__plugin_starcut_starcut__client_call` with `command: "get_transcript"` on each generated Audio Artifact.
5. If prepared, call `mcp__plugin_starcut_starcut__run_task` with each returned Task input.

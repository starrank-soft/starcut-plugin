...
- For a new graphic, use `mcp__plugin_starcut_starcut__run_task` with `task: "create_svg"` or
  `task: "create_mg"`.
...
copied from `mcp__plugin_starcut_starcut__head`. `create_mg` additionally accepts `durationUs`
...
If `run_task` returns `monitoring`, call `mcp__plugin_starcut_starcut__poll` with the exact

...
- Existing Nodes returned by `mcp__plugin_starcut_starcut__read` have immutable IDs. Never
  invent an ID.
- Omit IDs from complete VML supplied to `mcp__plugin_starcut_starcut__write` and subtrees
  supplied to `mcp__plugin_starcut_starcut__add_node`; StarCut assigns them.
...
- Use `mcp__plugin_starcut_starcut__add_node`, `mcp__plugin_starcut_starcut__update_node`,
  `mcp__plugin_starcut_starcut__move_node`, or `mcp__plugin_starcut_starcut__delete_node` for focused
...
A complete `mcp__plugin_starcut_starcut__write` uses the supplied VML exactly, so include that
...
There is no `speed` Attribute. Use `mcp__plugin_starcut_starcut__head` to inspect a media source before
...
Use exact paths returned by `mcp__plugin_starcut_starcut__glob`, `mcp__plugin_starcut_starcut__head`, current VML, or a completed
...
present in the conversation; otherwise call `mcp__plugin_starcut_starcut__query` with
...
- Use `mcp__plugin_starcut_starcut__update_node` for Clip or Track Attributes and for exact
  TextClip or CaptionClip Text Data replacement.
- Use `mcp__plugin_starcut_starcut__add_node` on a Composition to add a Track, or on a Track
  to add one compatible Clip. Use `beforeId` only for an intentional ordered
  insertion.
- Use `mcp__plugin_starcut_starcut__move_node` to reorder a Node or move it to a compatible
  parent. Never write internal owner or order fields.
- Use `mcp__plugin_starcut_starcut__delete_node` only after removing inbound references and

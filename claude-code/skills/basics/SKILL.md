...
1. Call `mcp__plugin_starcut_starcut__create_project` when the user wants a new project.
2. Call `mcp__plugin_starcut_starcut__list_projects` when an existing project is intended but
   its ID is unknown.
3. Project creation does not open the editor. When the host exposes a trusted
   ...
   returned by `create_project` the next action. Do not import, generate, edit,
   call another project tool, or ask a follow-up until the editor is confirmed
   loaded. If the host has no trusted browser surface, never print the handoff
   token; ask the user to open the stable `editorUrl` and continue after the
   editor is loaded.
4. Treat `browserHandoff.url` as a short-lived, one-time credential. Never
   ...
   same handoff again.
5. Call `mcp__plugin_starcut_starcut__open_project` to reopen an existing project or replace
...
`mcp__plugin_starcut_starcut__write` or `mcp__plugin_starcut_starcut__edit`. Markdown, SVG, and MG are editable
...
| Need | Tool |
|---|---|
| Find a project | `mcp__plugin_starcut_starcut__list_projects` |
| List project files or Artifacts | `mcp__plugin_starcut_starcut__glob` |
| Search editable file content | `mcp__plugin_starcut_starcut__grep` |
...
| Inspect live model capability | `mcp__plugin_starcut_starcut__query` |
...
Use `mcp__plugin_starcut_starcut__read` for known editable text. A VML Node can be read with
...
Use `mcp__plugin_starcut_starcut__head` for authoritative Artifact kind, readiness, dimensions,
...
Use `mcp__plugin_starcut_starcut__grep` for VML or Markdown source. VML matches can include
...
it. Use `mcp__plugin_starcut_starcut__update_project` for explicit name, description, or cover
changes:
...
Use `mcp__plugin_starcut_starcut__write` to create a complete Composition VML file:
...
`mcp__plugin_starcut_starcut__write` does not add one implicitly.
...
Use `mcp__plugin_starcut_starcut__add_node` to add one child or subtree to an existing parent:
...
Use `mcp__plugin_starcut_starcut__update_node` to change Attributes and/or one exact Text Data
...
Use `mcp__plugin_starcut_starcut__move_node` to reorder a child or move it to another compatible
parent. Omit `parentId` to keep the current parent, and omit `beforeId` to append.
Never write internal owner or order fields.

Use `mcp__plugin_starcut_starcut__delete_node` only for a non-root Node. Deletion includes its
descendants and is rejected when the remaining document would be invalid.

### Source Files

Use `mcp__plugin_starcut_starcut__write` to create a complete Markdown file under `docs/`, or
...
Markdown, SVG, and MG do not expose VML Nodes. Use `mcp__plugin_starcut_starcut__edit` for one
...
a `callId`; pass that `callId` to `mcp__plugin_starcut_starcut__poll`. Project file and Node
tools such as `glob`, `head`, `read`, `write`, and `edit` return a terminal
result and never return `monitoring`.

Use `mcp__plugin_starcut_starcut__client_call` for work that needs the connected Editor's

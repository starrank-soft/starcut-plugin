...
```text
call mcp__plugin_starcut_starcut__query for the requested capability
...
use `mcp__plugin_starcut_starcut__query` for the selected intent before constructing detailed
model parameters.
...
Locate a path with `mcp__plugin_starcut_starcut__glob`, inspect it with
`mcp__plugin_starcut_starcut__head`, and copy both fields. Never pass an Artifact path, ID
...
output, call `mcp__plugin_starcut_starcut__poll` with the exact `projectId` and `taskId`.

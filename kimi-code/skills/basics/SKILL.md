...
| Inspect models, fonts, or reusable Library/FX resources | `mcp__starcut__query` |
...
`mcp__starcut__poll`. `taskId` and `callId` remain compatibility aliases for
older clients, not separate identities for new workflows. Project file and
Node tools such as `mcp__starcut__glob`, `mcp__starcut__head`, `mcp__starcut__read`,

...
| Generate speech | `starcut_query` with `audio.tts`, then `starcut_run_task(generate)` |
...
`starcut_head`, and pass its exact Artifact ID:
...
Call `starcut_query` for `audio.asr` only when model selection matters.
...
`starcut_run_task` with `task: "transcribe"` on the returned ready Audio Artifact using its exact

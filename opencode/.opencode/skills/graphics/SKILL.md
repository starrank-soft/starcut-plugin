...
- For a new graphic, use `starcut_run_task` with `task: "create_svg"` or
  `task: "create_mg"`.
- Use `starcut_write` only when the current Agent deliberately authors
  the complete source itself.
- For an existing graphic, use `starcut_read` followed by
  `starcut_edit` for a focused change.
...
copied from `starcut_head`. `create_mg` additionally accepts `durationUs`
...
If `run_task` returns `monitoring`, call `starcut_poll` with the exact
...
current raw source, then call `starcut_edit` with one exact replacement:
...
`starcut_write` merely to update an existing graphic; reserve complete

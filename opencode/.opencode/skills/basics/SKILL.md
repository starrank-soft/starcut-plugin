...
| Inspect models, fonts, or reusable Library/FX resources | `starcut_query` |
...
`starcut_use_library` with that ID and the current `projectId`. Use a
...
`starcut_poll`. `taskId` and `callId` remain compatibility aliases for
older clients, not separate identities for new workflows. Project file and
Node tools such as `starcut_glob`, `starcut_head`, `starcut_read`,

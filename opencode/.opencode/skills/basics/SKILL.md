...
Run `opencode mcp auth starcut`, complete authorization in the browser, and start a new OpenCode session.

## Establish the Project

Use the exact `projectId` returned by StarCut for every project-scoped call.

1. Call `starcut_create_project` when the user wants a new project.
2. Call `starcut_list_projects` when an existing project is intended but
   its ID is unknown.
3. Project creation does not open the editor. When the host exposes a trusted
   ...
   returned by `starcut_create_project` the next action. Do not import,
...
4. Treat `browserHandoff.url` as a short-lived, one-time credential. Never
   ...
   same handoff again.
5. Call `starcut_open_project` to reopen an existing project or replace
...
`starcut_write` or `starcut_edit`. Markdown, SVG, and MG are editable
...
| Need | Tool |
|---|---|
| Find a project | `starcut_list_projects` |
| List project files or Artifacts | `starcut_glob` |
| Search editable file content | `starcut_grep` |
| Read one VML, Markdown, SVG, or MG file | `starcut_read` |
| Read one VML Node | `starcut_read` with `path` and `nodeId` |
| Inspect file or Artifact metadata | `starcut_head` |
| Inspect models, fonts, or reusable SFX | `starcut_query` |

Useful `starcut_glob` patterns include:
...
`starcut_use_library` with its `libraryId` and the current `projectId`,
...
it. Use `starcut_update_project` for explicit name, description, or cover
changes:
...
Use `starcut_write` to create a complete Composition VML file:
...
`starcut_write` does not add one implicitly.
...
Use `starcut_add_node` to add one child or subtree to an existing parent:
...
Use `starcut_update_node` to change Attributes and/or one exact Text Data
...
Use `starcut_move_node` to reorder a child or move it to another compatible
parent. Omit `parentId` to keep the current parent, and omit `beforeId` to append.
Never write internal owner or order fields.

Use `starcut_delete_node` only for a non-root Node. Deletion includes its
descendants and is rejected when the remaining document would be invalid.

### Source Files

Use `starcut_write` to create a complete Markdown file under `docs/`, or
...
Markdown, SVG, and MG do not expose VML Nodes. Use `starcut_edit` for one
...
and do not use `starcut_write` merely to change copy, color, timing, or
one animation detail. Load `graphics` before authoring or substantially
changing graphic source.

## Run Project Operations

Use `starcut_run_task` for media generation, SVG/MG generation, and ASR.
If it returns `monitoring`, call `starcut_poll` with the exact `projectId`
and returned `taskId`; never resubmit the same request merely because it is
still running.

Only `starcut_run_task` and `starcut_client_call` are pollable. A
deferred `starcut_client_call` returns a `callId`; pass that `callId` to
`starcut_poll`. Project file and Node tools such as
`starcut_glob`, `starcut_head`, `starcut_read`,
`starcut_write`, and `starcut_edit` return a terminal result and
never return `monitoring`.

Use `starcut_client_call` for work that needs the connected Editor's

...
1. Establish the exact project with `starcut_list_projects`,
   `starcut_create_project`, or `starcut_open_project`.
2. If `starcut_create_project` or `starcut_open_project` returns
...
3. Call `starcut_import_media` once with the exact `projectId`.
...
6. Use `starcut_glob` or `starcut_head` on the returned Artifact

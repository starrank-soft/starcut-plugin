...
| Generate speech | `starcut/query` with `kind: "model"` and intent `audio.tts`, then `starcut/run_task(generate)` |
...
`starcut/head`, and pass its exact path:
...
Call `starcut/query` with `kind: "model"` and intent `audio.asr` only
...
`starcut/run_task` with `task: "transcribe"` and the returned Audio path.

...
| Inspect models, fonts, or reusable Library/FX resources | `starcut/query` |
...
`starcut/use_library` with that ID and the current `projectId`. Use a
...
`starcut/poll`. `taskId` and `callId` remain compatibility aliases for
older clients, not separate identities for new workflows. Project file and
Node tools such as `starcut/glob`, `starcut/head`, `starcut/read`,

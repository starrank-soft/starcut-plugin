...
```text
call starcut/query for the requested capability
...
use `starcut/query` for the selected intent before constructing detailed
model parameters.
...
Locate a path with `starcut/glob`, inspect it with
`starcut/head`, and copy both fields. Never pass an Artifact path, ID

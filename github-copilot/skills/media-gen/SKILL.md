...
compatible `starcut/query` result already present in the conversation.
...
Locate a path with `starcut/glob` and inspect it with
`starcut/head`. Pass that path directly; never pass a URL, binary data,
...
`quality`; use only keys and values returned by `starcut/query`.
...
model-specific and must come from `starcut/query`.
...
If a result fits, call `starcut/use_library` with its `libraryId` and the
...
`starcut/run_task` returns stable Task and Artifact identities without
waiting for generation to finish. If the workflow depends on progress or
output, call `starcut/poll` with the exact `projectId` and returned
`toolCallId`.

...
1. Establish the exact project with `starcut/list_projects`,
   `starcut/create_project`, or `starcut/open_project`.
2. If `starcut/create_project` or `starcut/open_project` returns
...
3. Call `starcut/import_media` once with the exact `projectId`.
...
6. Use `starcut/glob` or `starcut/head` on the returned Artifact

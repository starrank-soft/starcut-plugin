...
- For a new graphic, use `starcut/run_task` with `task: "create_svg"` or
  `task: "create_mg"`.
- Use `starcut/write` only when the current Agent deliberately authors
  the complete source itself.
- For an existing graphic, use `starcut/read` followed by
  `starcut/edit` for a focused change.
...
copied from `starcut/head`. `create_mg` additionally accepts `durationUs`
...
If `run_task` returns `monitoring`, call `starcut/poll` with the exact
...
current raw source, then call `starcut/edit` with one exact replacement:
...
`starcut/write` merely to update an existing graphic; reserve complete

...
- Existing Nodes returned by `mcp__starcut__read` have immutable IDs. Never
  invent an ID.
- Omit IDs from complete VML supplied to `mcp__starcut__write` and subtrees
  supplied to `mcp__starcut__add_node`; StarCut assigns them.
...
- Use `mcp__starcut__add_node`, `mcp__starcut__update_node`,
  `mcp__starcut__move_node`, or `mcp__starcut__delete_node` for focused
...
A complete `mcp__starcut__write` uses the supplied VML exactly, so include that
...
call `mcp__starcut__use_library` with its `libraryId` and the current
...
There is no `speed` Attribute. Use `mcp__starcut__head` to inspect a media source before
...
Use exact paths returned by `mcp__starcut__glob`, `mcp__starcut__head`, current VML, or a completed
...
Query `fx`, choose a result, then call `mcp__starcut__use_library` with its
...
Use only a placement returned by `mcp__starcut__use_library`; never infer it
...
present in the conversation; otherwise call `mcp__starcut__query` with

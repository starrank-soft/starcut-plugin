...
```text
call mcp__starcut__query for the requested capability
...
use `mcp__starcut__query` for the selected intent before constructing detailed
model parameters.
...
Locate a path with `mcp__starcut__glob`, inspect it with
`mcp__starcut__head`, and copy both fields. Never pass an Artifact path, ID

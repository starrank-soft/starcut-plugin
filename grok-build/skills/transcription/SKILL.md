...
| Generate speech | `starcut__query` with `kind: "model"` and intent `audio.tts`, then `starcut__run_task(generate)` |
...
`starcut__head`, and pass its exact path:
...
Call `starcut__query` with `kind: "model"` and intent `audio.asr` only
...
`starcut__run_task` with `task: "transcribe"` and the returned Audio path.

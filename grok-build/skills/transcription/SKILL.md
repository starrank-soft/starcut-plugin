...
`starcut__poll` with its returned `toolCallId`; do not call
...
4. Call `starcut__client_call` with `command: "get_transcript"` on each generated Audio Artifact.
5. If prepared, call `starcut__run_task` with each returned Task input.

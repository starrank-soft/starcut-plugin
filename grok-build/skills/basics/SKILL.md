...
| Inspect models, fonts, or reusable SFX | `starcut__query` |
...
deferred `starcut__client_call` returns a `callId`; pass that `callId` to
`starcut__poll`. Project file and Node tools such as
`starcut__glob`, `starcut__head`, `starcut__read`,

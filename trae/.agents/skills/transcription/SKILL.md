...
| Generate speech | `query` with `audio.tts`, then `run_task(generate)` |
...
`poll` with its `taskId`; do not call `get_transcript` again to
poll or start duplicate work.

## Get a Transcript

Locate the source with `glob`, inspect it with
`head`, and pass its exact Artifact ID:
...
Call `query` for `audio.asr` only when model selection matters.
...
`run_task` with `task: "transcribe"` on the returned ready Audio Artifact using its exact
...
4. Call `client_call` with `command: "get_transcript"` on the generated Audio Artifact.
5. If prepared, call `run_task` with the returned Task input.

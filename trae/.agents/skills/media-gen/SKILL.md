...
```text
call query for the requested capability
...
use `query` for the selected intent before constructing detailed
model parameters.
...
Locate a path with `glob`, inspect it with
`head`, and copy both fields. Never pass an Artifact path, ID
